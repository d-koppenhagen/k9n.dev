...
description: 'With this blog post, I will show you how you can easily create an Angular blogging app by using the power of _Scully_ static site genrator. It will automatically detect all app routes and create static sites out of it that are ready to ship for production.'
publish: false
...
# Create powerfull fast pre-rendered Angular Apps using _Scully_ static site generator

**On _Dec 16, 2019_ the static site generator _Scully_ for Angular [has been presented](https://www.youtube.com/watch?v=Sh37rIUL-d4). _Scully_ will automatically detect all app routes and create static sites out of it that are ready to ship for production. With this blog post, I want to show you how you can easily create an Angular blogging app by using _Scully_ for pre-render your complete app.**
...
Scully will go analyze a built Angular app and detect all the routes of the app.
...
Et voilà here it is: We now have a very minimalistic Angular app that uses the power of _Scully_ to automatically find all app routes, visit them and generate static sites out of it.
...
After _Scully_ has checked our app, it will add the gernerated static sources to our `dist/static` directory by default.
...
If we have a look at it, except of the file `scully-routes.json`, that contains just an empty array, we don't see any differences between the two builds.
This is because we have just the root route configured curently and no more further content created.

Nonetheless we can checkout the result of the static sites in the browser by running `npm run scully:serve`. This will start two servers:

- `localhost:1668` : This server serves the static generated sites like a normal webserver (e.g. _nginx_ or _apache_)
...
Scully brings this feature out of the box and it’s fascinating easy to set it up:
...
We can simply delete all the content of `app.component.html` except of the last line.
So in the end the file `app.component.html` should just contain the `RouterOutlet`:
...
npm run scully        # generate static sites build
...
The files `index.html` in `dist/static/blog/<post-name>/` are containing our static sites ready to be served.
...
The `ScullyRoutesService` will return always all matching routes.
In fact if we are creating more routes in our app, it will detect them and the result of the `available$` observable will contain their data.
...
Special thanks go to [Aaron Frost (Frosty ⛄️)](https://twitter.com/aaronfrost) from the _Scully_ core team for revising this article.
