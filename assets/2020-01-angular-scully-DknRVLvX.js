...
<pre><code>"@angular/core": "~13.0.0",
"@angular/cli": "~13.0.3",
"@scullyio/init": "^2.0.5",
"@scullyio/ng-lib": "^2.0.0",
"@scullyio/scully": "^2.0.0",
"@scullyio/scully-plugin-puppeteer": "^2.0.0",</code></pre></blockquote>
<hr>
...
<p>But that’s not all: Once the pre-rendered page is shipped to the user, <em>Scully</em> loads and bootstraps the “real” Angular app  in the background on top of the existing view.
...
Therefore, we add the appropriate flag <code>--routing</code> (we can also choose this option when the CLI prompts us).</p>
...
Therefore, we are using the provided Angular schematic:</p>
<pre class="shiki github-dark" style="background-color:#24292e;color:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#B392F0">ng</span><span style="color:#9ECBFF"> add</span><span style="color:#9ECBFF"> @scullyio/init</span><span style="color:#6A737D">  # add Scully to the project</span></span></code></pre>
...
Let&#39;s try it out by building our site and running <em>Scully</em>.</p>
<pre class="shiki github-dark" style="background-color:#24292e;color:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#B392F0">npm</span><span style="color:#9ECBFF"> run</span><span style="color:#9ECBFF"> build</span><span style="color:#6A737D">     # build our Angular app</span></span>
...
Let&#39;s quickly compare the result generated from <em>Scully</em> with the result from the initial Angular build (<code>dist/scully-blog</code>):</p>
<pre><code>dist/
┣ scully-blog/
┃ ┣ assets/
┃ ┣ ...
...
┗ static/
  ...
  ┃ ┗ scully-routes.json
  ┣ ...
  ┗ styles.ef46db3751d8e999.css</code></pre><p>If we take a look at it, except of the file <code>scully-routes.json</code>, that contains the configured routes used by <em>Scully</em>, we don&#39;t see any differences between the two builds.
This is because currently we only have the root route configured, and no further content was created.</p>
<p>Nonetheless, when running <code>npx scully serve</code> or <code>npx scully --watch</code> we can check out the result by visiting the following URL: <code>localhost:1668</code>.
...
<h2 id="the-scullylibmodule">The <code>ScullyLibModule</code></h2>
<p>You may have realized, that after running the <em>Scully</em> schematic, the <code>ScullyLibModule</code> has been added to your <code>AppComponent</code>:</p>
...
<h2 id="turn-it-into-a-blog">Turn it into a blog</h2>
...
<em>Scully</em> brings this feature out of the box, and it’s very easy to set it up:</p>
<pre class="shiki github-dark" style="background-color:#24292e;color:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#B392F0">ng</span><span style="color:#9ECBFF"> g</span><span style="color:#9ECBFF"> @scullyio/init:blog</span><span style="color:#6A737D">                      # setup up the \`BlogModule\` and related sources</span></span>
...
We can simply delete all the content of <code>app.component.html</code> except of the <code>router-outlet</code>:</p>
<pre class="shiki github-dark" style="background-color:#24292e;color:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#E1E4E8">&#x3C;</span><span style="color:#85E89D">router-outlet</span><span style="color:#E1E4E8">>&#x3C;/</span><span style="color:#85E89D">router-outlet</span><span style="color:#E1E4E8">></span></span></code></pre>
...
To be sure it will re-scan the routes, we will pass through the parameter <code>--scan</code>:</p>
<pre class="shiki github-dark" style="background-color:#24292e;color:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#B392F0">npm</span><span style="color:#9ECBFF"> run</span><span style="color:#9ECBFF"> build</span><span style="color:#6A737D">       # Angular build</span></span>
...
But what&#39;s that: When we will check the directory <code>dist/static/blog/</code>, we see somewhat like this:</p>
<pre><code>blog/
┣ ___UNPUBLISHED___k9pg4tmo_2DDScsUiieFlld4R2FwvnJHEBJXcgulw
  ┗ index.html</code></pre><p>This feels strange, doesn&#39;t it?
...
But normally you would just like to change the metadata in the Markdown file to:</p>
<pre class="shiki github-dark" style="background-color:#24292e;color:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#85E89D">published</span><span style="color:#E1E4E8">: </span><span style="color:#79B8FF">true</span></span></code></pre>
...
<p>If you want to prove that the page is actually really pre-rendered, just disable JavaScript by using your Chrome Developer Tools.
You can reload the page and see that the content is still displayed.
Awesome, isn&#39;t it?</p>
<p><img src="images/blog/scully/scully-pre-rendered-js-disabled.png" alt="a simple blog created with Scully"></p>
<blockquote>
...
<p>You may have realized: We haven’t written one line of code manually yet, and we have already a fully functional blogging site that’s server site rendered. Isn’t that cool?
...
We can easily inject the service and display the information as a list in our <code>AppComponent</code>.</p>
<pre class="shiki github-dark" style="background-color:#24292e;color:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#F97583">import</span><span style="color:#E1E4E8"> { Component } </span><span style="color:#F97583">from</span><span style="color:#9ECBFF"> '@angular/core'</span><span style="color:#E1E4E8">;</span></span>
...
We can extend <code>app.component.html</code> like this:</p>
<pre class="shiki github-dark" style="background-color:#24292e;color:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#E1E4E8">&#x3C;</span><span style="color:#85E89D">ul</span><span style="color:#E1E4E8">></span></span>
...
<p><img src="images/blog/scully/scully-blog.gif" alt="a simple blog created with scully"></p>
<p>The <code>ScullyRoutesService</code> contains all the available routes in your app.
...
To list only blog posts from the <code>blog</code> route and directory we can just filter the result:</p>
<pre class="shiki github-dark" style="background-color:#24292e;color:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#6A737D">/* ... */</span></span>
...
<h2 id="fetch-dynamic-information-from-an-api">Fetch dynamic information from an API</h2>
...
This has been configured from the schematics we ran before in the file <code>scully.scully-blog.config.ts</code>:</p>
<pre class="shiki github-dark" style="background-color:#24292e;color:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#F97583">import</span><span style="color:#E1E4E8"> { ScullyConfig } </span><span style="color:#F97583">from</span><span style="color:#9ECBFF"> '@scullyio/scully'</span><span style="color:#E1E4E8">;</span></span>
<span class="line"></span>
...
We can configure <a href="https://scully.io/docs/plugins#router-plugin"><em>Router Plugin</em></a> that will call an API, fetch the data from it and pluck the <code>isbn</code> from the array of results to fill it in the router parameter.</p>
<p>In the following example we will use the public service <a href="https://api3.angular-buch.com">BookMonkey API</a> (we provide this service for the readers of our <a href="https://angular-buch.com/">German Angular book</a>) as an API to fetch a list of books:</p>
<pre class="shiki github-dark" style="background-color:#24292e;color:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#6A737D">/* ... */</span></span>
<span class="line"></span>
<span class="line"><span style="color:#F97583">export</span><span style="color:#F97583"> const</span><span style="color:#79B8FF"> config</span><span style="color:#F97583">:</span><span style="color:#B392F0"> ScullyConfig</span><span style="color:#F97583"> =</span><span style="color:#E1E4E8"> {</span></span>
<span class="line"><span style="color:#6A737D">  /* ... */</span></span>
...
<span class="line"><span style="color:#E1E4E8">  }</span></span>
<span class="line"><span style="color:#E1E4E8">};</span></span></code></pre>
<p>The result from the API will have this shape:</p>
...
In fact, when running <em>Scully</em> using <code>npx scully</code>, it will visit the following routes, <strong>after we have configured the route <code>/books/:isbn</code> in the Angular router</strong> (otherwise non-used routes will be skipped).</p>
<pre><code>/books/9783864906466
/books/9783864903571</code></pre><p>We can see the result in the log:</p>
<pre><code>enable reload on port 2667
...
Generating took 3.3 seconds for 7 pages:
  ...
  Rendering the pages took 2.58 seconds</code></pre><p>This is great. We have efficiently pre-rendered normal dynamic content!
...
<a href="https://github.com/d-koppenhagen/k9n.dev">github.com/d-koppenhagen/k9n.dev</a></p>
</blockquote>
<p>If you want to follow all the development steps in detail, check out my provided GitHub repository
<a href="https://github.com/d-koppenhagen/scully-blog-example">scully-blog-example</a>.</p>
...
<p>Compared to &quot;classic&quot; pre-rending by using <a href="https://angular.io/guide/universal">Angular Universal</a>, <em>Scully</em> is much easier to use, and it doesn&#39;t require you to write a specific flavor of Angular.
...
If you want to compare <em>Scully</em> with Angular Universal in detail, check out the blog post from Sam Vloeberghs: <a href="https://samvloeberghs.be/posts/scully-or-angular-universal-what-is-the-difference">Scully or Angular Universal, what is the difference?</a></p>
<p>If you want to dig a bit deeper into the features <em>Scully</em> offers, check out my <a href="/blog/2020-03-dig-deeper-into-scully-ssg">second article</a>.</p>
<p><strong>Thank you</strong></p>
<p>Special thanks go to <a href="https://twitter.com/aaronfrost">Aaron Frost (Frosty ⛄️)</a> from the <em>Scully</em> core team, <a href="https://twitter.com/fmalcher01">Ferdinand Malcher</a> and <a href="https://twitter.com/JohannesHoppe">Johannes Hoppe</a> for revising this article.</p>
`;export{e as default};
